...
class KnowledgeBase:
    ...
    def preprocess_knowledge(self, texts, save_path):
        ...
        self.knowledge_embeddings = model(texts)
    ...
    def match_query(self, queries, top_k=3):
        ...
        if scores.ndim == 1:
            print('进入到score中，进行增加维度。')
            scores = scores[None, :]  # 变成 (1, M)
...
def init():
    ...
    model_dir = "/Volumes/mac_win/models/BAAI/bge-small-zh-v1.5"
    instruction = "为这个句子生成表示以用于检索相关文章"
    # 初始化并加载知识库
    knowledge_base = KnowledgeBase(model_dir, instruction)
    knowledge_base.load_knowledge("/Volumes/PSSD/未命名文件夹/fortune_teller-main/RAG/data/knowledge_embeddings_with_indices.pkl")
    a = knowledge_base.match_query('nihao',3)
    print(a)
